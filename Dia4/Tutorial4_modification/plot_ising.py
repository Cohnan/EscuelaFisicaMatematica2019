...
T_list = [2.269]#np.linspace(5.0,0.5,50) #temperature list
...
for (iT,T) in enumerate(T_list):
  file = open('Data_ising2d/RBM_samples/samples_nH4_L%d_T%.3f.txt' %(L,T), 'r')
  data = np.loadtxt( file )

  E   = data[:,1] # For each temperature we did n_measSweeps sweeps: that is our sample
  ##print(type(E))
  ##print(E)
  ##print(np.mean(E))
  ##print(np.mean(E**2))
  ...
  # ***********               AND SUSCEPTIBILITY                *********** #
  # *********************************************************************** #
  specHeat[iT] = 1./N_spins * (np.mean(E**2) - energy[iT]**2)
  susc[iT]     = 1./N_spins * (np.mean(M**2) - mag[iT]**2)
#end loop over T
...
#plt.xlabel('$T$')
#plt.ylabel('$<E>/N$')
print('<E>/N: ', energy[0]/(1.0*N_spins))
...
#plt.xlabel('$T$')
#plt.ylabel('$C_V/N$')
print('C_V/N:', specHeat[0]/(1.0*N_spins))
...
#plt.xlabel('$T$')
#plt.ylabel('$\chi/N$')
print('\chi/N:', susc[0]/(1.0*N_spins))
